...
def process_dna(fasta_file):
    ...
    for i in range(len(seqs)):
        ...
        for key in open_reading_frames:
            head = f"{heads[i]}.{key}"
            dna = process_strand(coding[key])
            proteins.update({head: str(Seq(dna).translate())})
            ...
            inst = instable(dna)
            mw = weight(dna)
            se_st = sec_struct(dna).split(",")
            se_st1 = se_st[0]
            se_st2 = se_st[1]
            se_st3 = se_st[2]
            me = mol_ext(dna).split(",")
            me1 = me[0]
            me2 = me[1]
            n = pd.DataFrame(
                {
...
data = pd.read_csv("./data/homosapiens.csv")
print("Loaded data")
